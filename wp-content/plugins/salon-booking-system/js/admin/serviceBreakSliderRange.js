...
function sln_serviceBreakSliderRange($, $elements) {
    ...
    $('#_sln_service_break_duration_enabled').on('change', function () {
        ...
        if ($(this).prop('checked')) {
            if ( ! getMinutesDuration($('.slider-time-input-break-duration').val()) ) {
                $('.slider-time-input-break-duration').val(getDuration(+$('.slider-time-input-step').val()));
                $('.slider-time-input-from').val(parseInt(+$('.slider-time-input-step').val() / 2));
                $('.slider-time-input-to').val(parseInt(+$('.slider-time-input-step').val() / 2) + +$('.slider-time-input-step').val());
            }
            initSliderRange($elements);
        } else {
            $('.slider-time-input-break-duration').val("00:00");
            $('.slider-time-input-from').val(0);
            $('.slider-time-input-to').val(0);
