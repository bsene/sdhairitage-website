...
class SLN_Action_InitScripts {
	...
	public function hook_enqueue_scripts() {
	    global $post;
	    if ( !$this->isAdmin && is_a( $post, 'WP_Post' ) && ( has_shortcode( $post->post_content, SLN_Shortcode_Salon::NAME ) || has_shortcode( $post->post_content, SLN_Shortcode_SalonMyAccount::NAME ) || has_shortcode( $post->post_content, SLN_Shortcode_SalonCalendar::NAME ) || has_shortcode( $post->post_content, SLN_Shortcode_SalonAssistant::NAME ) || has_shortcode( $post->post_content, SLN_Shortcode_SalonServices::NAME ) || has_shortcode( $post->post_content, SLN_Shortcode_SalonRecentComments::NAME ) ) ) {
		self::preloadScripts();
